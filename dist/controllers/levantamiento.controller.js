import { crearLevantamientoService, listarLevantamientosService, obtenerLevantamientoPorIdService, obtenerLevantamientoPorSiniestroService, actualizarLevantamientoService, agregarConceptoService, eliminarConceptoService, eliminarLevantamientoService, calcularCostoTotalService, } from "../services/levantamiento.service.js";
export async function crearLevantamiento(req, res) {
...
export async function listarLevantamientos(req, res) {
    const { no_siniestro } = req.query;
    const result = await listarLevantamientosService(no_siniestro);
    return res.json(result);
}
export async function obtenerLevantamientoPorId(req, res) {
...
export async function obtenerLevantamientoPorSiniestro(req, res) {
...
export async function actualizarLevantamiento(req, res) {
    const id = Number(req.params.id);
    if (isNaN(id)) {
        return res.status(400).json({ error: "id inválido" });
    }
...
export async function agregarConcepto(req, res) {
...
export async function eliminarConcepto(req, res) {
...
export async function eliminarLevantamiento(req, res) {
    const id = Number(req.params.id);
    if (isNaN(id)) {
        return res.status(400).json({ error: "id inválido" });
    }
...
export async function calcularCostoTotal(req, res) {
    const id = Number(req.params.id);
    if (isNaN(id)) {
        return res.status(400).json({ error: "id inválido" });
    }
