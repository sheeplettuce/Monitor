...
function crearCarpetaFormatos(no_siniestro) {
...
export async function generarFormatoService(no_siniestro, carpetaDestino) {
    try {
        ...
        if (!ws)
            return { error: "La plantilla no contiene hojas de cálculo" };
...
const CHECKLIST_ITEMS = [
    ...
    ...["Liquido de freno", "Efectividad de frenado", "Freno motor/escape", "Freno estacionario"].map((nombre_item, i) => ({
    ...
    ...["Llantas de repuesto", "Llantas delanteras", "Llantas posteriores", "Esparragos y tuercas"].map((nombre_item, i) => ({
...
const CHECKLIST_MAP = new Map(CHECKLIST_ITEMS.map((item) => [
    `${normalizar(item.sistema)}|${normalizar(item.nombre_item)}`,
    item,
]));
const VALOR_INDEX = { OK: 0, RE: 1, MA: 2, NA: 3 };
export async function generarChecklistService(no_siniestro, carpetaDestino) {
    try {
        ...
        if (!checklist)
            return { error: "Checklist no encontrado para este siniestro" };
...
export async function generarLevantamientoService(no_siniestro, carpetaDestino) {
    try {
        ...
        if (!levantamiento)
            return { error: "Levantamiento de daños no encontrado para este siniestro" };
        ...
        conceptos.forEach((c, i) => {
            ...
            if (!slot)
                return; // se excede la capacidad de la plantilla
 ...
 */
export async function generarFormatosParaRespaldo(no_siniestro, carpetaDestino) {
