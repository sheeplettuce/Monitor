...
export async function checkDatabase(prisma) {
    try {
...
export function checkDisk() {
    try {
        let libre_GB, usado_GB, total_GB;
        if (isWindows) {
            // wmic devuelve bytes, convertimos a GB
            const result = execSync(`wmic logicaldisk where "DeviceID='C:'" get FreeSpace,Size /format:csv`, { encoding: "utf8" });
