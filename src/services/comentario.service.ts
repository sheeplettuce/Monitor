 ...
 */
export async function listarComentariosPorSiniestroService(no_siniestro: string) {
  try {
 ...
 */
export async function eliminarComentarioService(id: number) {
  try {
    const existe = await prisma.comentario.findUnique({ where: { id } });
    if (!existe) return { error: "Comentario no encontrado" };
