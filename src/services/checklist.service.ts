...
function dbError(e: unknown, contexto: string) {
  logger.error("Checklist", contexto, e);
  return { error: "Error de base de datos" };
}

function normalizarChecklist(data: any) {
  return {
    ...data,
...
export async function crearChecklistService(data: any) {
  try {
...
export async function obtenerChecklistPorIdService(id: number) {
  try {
...
export async function obtenerChecklistPorSiniestroService(no_siniestro: string) {
  try {
...
export async function actualizarChecklistService(id: number, data: any) {
  try {
    const existe = await prisma.checklist.findUnique({
      where: { id },
    });

    if (!existe) return { error: "Checklist no encontrado" };
...
export async function agregarItemService(id_checklist: number, data: any) {
  try {
    const existe = await prisma.checklist.findUnique({
...
export async function actualizarItemsChecklistService(
  id_checklist: number,
  items: any[]
) {
  try {
...
export async function eliminarItemService(id_item: number) {
  try {
    const existe = await prisma.checklist_item.findUnique({
      where: { id: id_item },
    });

    if (!existe) return { error: "Item no encontrado" };
...
export async function eliminarChecklistService(id: number) {
  try {
    const existe = await prisma.checklist.findUnique({
      where: { id },
    });
